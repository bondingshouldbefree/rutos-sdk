			   ...
			   struct tcb_cb_data *cbd)
{
	...
	char *p, *payload_ptr, *uri_ptr, *fields_ptr, *tmp_ptr;
	...
	tmp_ptr = strstr(pdata->buf, content_type_str);
	if (tmp_ptr) {
		tmp_ptr = strstr(tmp_ptr, boundary_str);
		if (tmp_ptr) {
			tmp_ptr = strstr(payload_ptr + 4,"\r\n\r\n");
			if (!tmp_ptr) {
				/* not fully received, waiting for next rx */
				if (size_rcvd == cbd->datalen)
					return 1;
	...
	*p = 0;
	...
	if (method == HTTP_POST) {
		...
		pdata->proc_size = hdr_size + pdata->upload_size; 
			       ...
			       struct tcb_cb_data *cbd)
{
	...
	if (cbd->datalen > pdata->proc_size)
	{
