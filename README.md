...
The helper script is available at: `RUTM_R_GPL_00.07.21.1/scripts/dockerbuild`.
...
2. Extract the  archive to an empty folder
   ...
   ```sh
   mkdir RUTM_R_GPL_00.07.21.1
   tar -xzf ~/Downloads/RUTM_R_GPL_00.07.21.1.tar.gz -C RUTM_R_GPL_00.07.21.1
   ```

3. Update feeds

   ```sh
   cd RUTM_R_GPL_00.07.21.1
...
5. Prepare packages for offline installation (optional)
   ...
   After this you can find packages in "RUTM_R_GPL_00.07.21.1/bin/packages/<arch_name>/zipped_packages".
...
After successful build you will get the firmware file in `RUTM_R_GPL_00.07.21.1/bin/targets/ramips/generic/tltFws`.
...
```text
RUTM_R_GPL_00.07.21.1/package/feeds/vuci/vuci-ui-core/bin/dist
```
...
2. Generate banner text:
   ...
   ```sh
   figlet YOUR_TEXT > "RUTM_R_GPL_00.07.21.1/package/base-files/files/etc/banner.logo"
   ```
...
1. Open the device family file:
   ...
   ```text
   RUTM_R_GPL_00.07.21.1/target/linux/ramips/image/devices/rutmxx_family.mk
   ```
...
4. Regenerate tmpinfo files:
   ...
   ```sh
   rm -rf "RUTM_R_GPL_00.07.21.1/tmp"
   ```
...
```text
RUTM_R_GPL_00.07.21.1/package/base-files/files/lib/preinit/84_set_password
```
