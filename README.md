...
The helper script is available at: `RUTX_R_GPL_00.07.17/scripts/dockerbuild`.
...
2. Extract the  archive to an empty folder
   ...
   ```sh
   mkdir RUTX_R_GPL_00.07.17
   tar -xzf ~/Downloads/RUTX_R_GPL_00.07.17.tar.gz -C RUTX_R_GPL_00.07.17
   ```

3. Update feeds

   ```sh
   cd RUTX_R_GPL_00.07.17
...
5. Prepare packages for offline installation (optional)
   ...
   After this you can find packages in "RUTX_R_GPL_00.07.17/bin/packages/<arch_name>/zipped_packages".
...
After successful build you will get the firmware file in `RUTX_R_GPL_00.07.17/bin/targets/ipq40xx/generic/tltFws`.
...
```text
RUTX_R_GPL_00.07.17/package/feeds/vuci/vuci-ui-core/bin/dist
```
...
2. Generate banner text:
   ...
   ```sh
   figlet YOUR_TEXT > "RUTX_R_GPL_00.07.17/package/base-files/files/etc/banner.logo"
   ```
...
1. Open the device family file:
   ...
   ```text
   RUTX_R_GPL_00.07.17/target/linux/ipq40xx/image/devices/rutxxx_family.mk
   ```
...
```text
RUTX_R_GPL_00.07.17/package/base-files/files/lib/preinit/84_set_password
```
